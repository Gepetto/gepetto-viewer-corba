...
#define TYPE_PROBE(type) \
  TYPE_PROBE_PROXY(TYPE_##type)  // concatenate prefix with user name
...
#define WRITE_INPUT_ARGS_2(T0, T1) \
  WRITE_INPUT_ARGS_1(T0), WRITE_INPUT_ARG(T1, 1)
#define WRITE_INPUT_ARGS_3(T0, T1, T2) \
  WRITE_INPUT_ARGS_2(T0, T1), WRITE_INPUT_ARG(T2, 2)
#define WRITE_INPUT_ARGS_4(T0, T1, T2, T3) \
  WRITE_INPUT_ARGS_3(T0, T1, T2), WRITE_INPUT_ARG(T3, 3)
#define WRITE_INPUT_ARGS_5(T0, T1, T2, T3, T4) \
  WRITE_INPUT_ARGS_4(T0, T1, T2, T3), WRITE_INPUT_ARG(T4, 4)
#define WRITE_INPUT_ARGS_6(T0, T1, T2, T3, T4, T5) \
  WRITE_INPUT_ARGS_5(T0, T1, T2, T3, T4), WRITE_INPUT_ARG(T5, 5)
...
#define WRITE_OP_ARGS_4(T0, T1, T2, T3) \
  WRITE_OP_ARGS_3(T0, T1, T2), WRITE_OP_ARG(T3, 3)
#define WRITE_OP_ARGS_5(T0, T1, T2, T3, T4) \
  WRITE_OP_ARGS_4(T0, T1, T2, T3), WRITE_OP_ARG(T4, 4)
#define WRITE_OP_ARGS_6(T0, T1, T2, T3, T4, T5) \
  WRITE_OP_ARGS_5(T0, T1, T2, T3, T4), WRITE_OP_ARG(T5, 5)
...
#define BIND_TO_WINDOWS_MANAGER_4(Ret, Func, T0, T1, T2, T3)     \
  WRITE_RET_T(Ret)                                               \
  GraphicalInterface::Func(WRITE_INPUT_ARGS_4(T0, T1, T2, T3)) { \
    try {                                                        \
      WRITE_RET_OP(Ret)                                          \
      (windowsManager_->Func(WRITE_OP_ARGS_4(T0, T1, T2, T3)));  \
    } catch (const std::exception& exc) {                        \
      throw Error(exc.what());                                   \
    }                                                            \
  }
...
#define BIND_TO_WINDOWS_MANAGER_5(Ret, Func, T0, T1, T2, T3, T4)     \
  WRITE_RET_T(Ret)                                                   \
  GraphicalInterface::Func(WRITE_INPUT_ARGS_5(T0, T1, T2, T3, T4)) { \
    try {                                                            \
      WRITE_RET_OP(Ret)                                              \
      (windowsManager_->Func(WRITE_OP_ARGS_5(T0, T1, T2, T3, T4)));  \
    } catch (const std::exception& exc) {                            \
      throw Error(exc.what());                                       \
    }                                                                \
  }
...
#define BIND_TO_WINDOWS_MANAGER_6(Ret, Func, T0, T1, T2, T3, T4, T5)     \
  WRITE_RET_T(Ret)                                                       \
  GraphicalInterface::Func(WRITE_INPUT_ARGS_6(T0, T1, T2, T3, T4, T5)) { \
    try {                                                                \
      WRITE_RET_OP(Ret)                                                  \
      (windowsManager_->Func(WRITE_OP_ARGS_6(T0, T1, T2, T3, T4, T5)));  \
    } catch (const std::exception& exc) {                                \
      throw Error(exc.what());                                           \
    }                                                                    \
  }
...
GraphicalInterface::GraphicalInterface(corba::Server* server)
    : windowsManager_(server->windowsManager()), qparent_(NULL) {}
...
BIND_TO_WINDOWS_MANAGER_3(BOOL, setLineExtremalPoints, STRING, POSITION,
                          POSITION)
...
BIND_TO_WINDOWS_MANAGER_5(BOOL, addTriangleFace, STRING, POSITION, POSITION,
                          POSITION, COLOR)

BIND_TO_WINDOWS_MANAGER_6(BOOL, addSquareFace, STRING, POSITION, POSITION,
...
BIND_TO_WINDOWS_MANAGER_5(BOOL, createRoadmap, STRING, COLOR, FLOAT, FLOAT,
                          COLOR)
