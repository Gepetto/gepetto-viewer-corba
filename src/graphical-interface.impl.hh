...
class GraphicalInterface
 ...
 public:
  ...
  void qparent(QObject* parent) { qparent_ = parent; }
  ...
  virtual bool addSceneToWindow(const char* sceneNameCorba,
                                const char* windowId);
  ...
  virtual bool addBox(const char* boxName, const float boxSize1,
                      const float boxSize2, const float boxSize3,
                      const Color color);

  virtual bool addCapsule(const char* capsuleName, float radius, float height,
                          const Color color);

  virtual bool addArrow(const char* arrowNameCorba, float radius, float length,
                        const Color colorCorba);

  virtual bool addRod(const char* rodNameCorba, const Color colorCorba,
  ...
  virtual bool resizeArrow(const char* arrowNameCorba, float newRadius,
                           float newLength);
  ...
  virtual bool addCone(const char* coneName, float radius, float height,
                       const Color color);

  virtual bool addCylinder(const char* cylinderName, float radius, float height,
                           const Color color);

  virtual bool addSphere(const char* sphereName, float radius,
                         const Color color);

  virtual bool addLight(const char* lightName, const char* windowId,
                        float radius, const Color color);

  virtual bool addLine(const char* lineName, const value_type* pos1,
                       const value_type* pos2, const Color color);
  ...
  virtual bool setLineExtremalPoints(const char* lineName,
                                     const value_type* pos1,
                                     const value_type* pos2);

  virtual bool addCurve(const char* curveName, const PositionSeq& pos,
                        const Color color);
  ...
  virtual bool setCurvePointsSubset(const char* curveName, CORBA::Long first,
                                    CORBA::Long count);
  virtual bool setCurveLineWidth(const char* curveName, const float width);

  virtual bool addSquareFace(const char* faceName, const value_type* pos1,
                             const value_type* pos2, const value_type* pos3,
                             const value_type* pos4, const Color color);
  virtual bool setTexture(const char* nodeName, const char* filename);
  virtual bool addTriangleFace(const char* faceName, const value_type* pos1,
                               const value_type* pos2, const value_type* pos3,
                               const Color color);

  virtual bool addXYZaxis(const char* nodeNameCorba, const Color colorCorba,
                          float radius, float sizeAxis);

  virtual bool createRoadmap(const char* nameCorba, const Color colorNodeCorba,
                             float radius, float sizeAxis,
                             const Color colorEdgeCorba);

  virtual bool addEdgeToRoadmap(const char* nameRoadmap,
                                const value_type* posFrom,
                                const value_type* posTo);

  virtual bool addNodeToRoadmap(const char* nameRoadmap,
                                const value_type* configuration);
  ...
  virtual void addUrdfObjects(const char* urdfNameCorba,
                              const char* urdfPathCorba, bool visual);

  virtual bool createGroup(const char* groupNameCorba);
  virtual bool addToGroup(const char* nodeNameCorba,
                          const char* groupNameCorba);
  virtual bool removeFromGroup(const char* nodeNameCorba,
                               const char* groupNameCorba);

  virtual bool removeObjectFromCache(const char* nodeNameCorba);
  virtual bool applyConfiguration(const char* nodeNameCorba,
                                  const value_type* configuration);
  virtual bool applyConfigurations(const Names_t& nodeNameCorba,
                                   const TransformSeq& configuration);
  ...
  virtual bool setStaticTransform(const char* nodeName,
                                  const Transform transform);

  virtual bool setVisibility(const char* nodeNameCorba,
  ...
  virtual bool setWireFrameMode(const char* nodeNameCorba,
                                const char* wireFrameModeCorba);
  virtual bool setLightingMode(const char* nodeNameCorba,
  ...
  virtual bool setCaptureTransform(const char* filename,
                                   const Names_t& nodename);
  virtual void captureTransformOnRefresh(bool autoCapture);
  virtual void captureTransform();
  virtual bool writeBlenderScript(const char* filename,
                                  const Names_t& nodeNames);
  ...
  virtual bool setCameraTransform(const char* windowId,
  ...
  virtual void setStringProperty(const char* nodeName, const char* propName,
                                 const char* value);
  virtual Color_slice* getColorProperty(const char* nodeName,
                                        const char* propName);
  virtual void setColorProperty(const char* nodeName, const char* propName,
                                const Color value);
  virtual Position_slice* getVector3Property(const char* nodeName,
                                             const char* propName);
  virtual void setVector3Property(const char* nodeName, const char* propName,
                                  const Position value);
  virtual Position_slice* getVector2Property(const char* nodeName,
                                             const char* propName);
  virtual void setVector2Property(const char* nodeName, const char* propName,
                                  const Position value);
  virtual float getFloatProperty(const char* nodeName, const char* propName);
  virtual void setFloatProperty(const char* nodeName, const char* propName,
                                float value);
  virtual bool getBoolProperty(const char* nodeName, const char* propName);
  virtual void setBoolProperty(const char* nodeName, const char* propName,
                               bool value);
  virtual CORBA::Long getIntProperty(const char* nodeName,
                                     const char* propName);
  virtual void setIntProperty(const char* nodeName, const char* propName,
                              CORBA::Long value);
